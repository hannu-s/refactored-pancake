...
with tf.Session() as session:
    ...
    while step < training_iters:
        ...
        _, acc, lass, onehot_pred = session.run([optimizer, accuracy, cost, pred],
        feed_dict={x: symbols_in_keys, y: symbols_out_onehot})
        loss_total += lass
        acc_total += acc
        if (step+1) % display_step == 0:
            print("iter= " + str(step+1) + ", avarage loss= {:.6f}".format(loss_total/display_step)
            + ", avarage accuracy= {:.2f}".format(100*acc_total/display_step))
            acc_total = 0
            loss_total = 0
            ...
            saved_session_path = "/tmp/ties4911_t5_s{0}/model_{0}.ckpt".format(step+1)
            save_path = saver.save(session, saved_session_path)
            print(save_path)
        step += 1
        offset += (n_input+1)
    #save_path = saver.save(session, saved_session_path)
